    ...
    namespace LalibeFH4QBlockEnv
    {
        ...
        void  InlineMeas::operator()(unsigned long update_no, XMLWriter& xml_out)
        {
           ...
           // Make an action and all other stuff needed for a solver.
            ...
            Bilinear_Gamma(params.fhparam.currents[0], fh_prop_src_b, quark_propagator, u);
            ...
            */
            ...
            QDPIO::cout << "FH_4QOPERATOR BBB: " << std::endl;
            k=0;


            //for(int s1=0;s1<1;s1++){
            int s1=params.fhparam.spins[0];
            if(true){
                for(int s2=0;s2<4;s2++){
                    ...
                    for(int c1=0;c1<3;c1++){ 
                        for(int c2=0;c2<3;c2++){
                            ...
                            QDPIO::cout << "First set of solves: s1=" <<s1 <<" s2="<<s2<<" c1=" <<c1 <<" c2=" <<c2 << std::endl;
                            ...
                            {
#if 0
                                action->quarkProp(fh_solutions[k], xml_out, fh_prop_src_c, t0, j_decay, action_state,
                            ...
                            {
                                QDPIO::cerr << "Solve failed: s1=" <<s1 <<" s2="<<s2<<" c1=" <<c1 <<" c2=" <<c2 << std::endl;                                 }
                            ...
                            */
                            ...
                            current_id = params.named_obj.fh_block_id+"_s"+ std::to_string(s1)+
                                                      +"_s"+std::to_string(s2);
                            current_id = current_id+"_c"+ std::to_string(c1)+"_c"+
                                                      std::to_string(c2);
                            QDPIO::cout << current_id << " k= "<<k<<std::endl;
                            TheNamedObjMap::Instance().create<LatticePropagator>(current_id);
                            TheNamedObjMap::Instance().getData<LatticePropagator>(current_id) = fh_prop_src_c;//fh_solutions[k];
                            ...
                            for(int i=0; i<144;i++){
                                ...
                                //QDPIO::cout << "cc["<<i<<"]=" << cc.elem(0).elem().elem().real()<<","<<cc.elem(0).elem().elem().imag()<<std::endl;
                                pokeColor(dest,cc,c1,c2);
                                //pokeSpin(fh_solutions[i],dest,s1,s2);

                                //QDPIO::cout << "fh_solutions["<<i<<"]=" << fh_solutions[i].elem(0).elem(s1,s2).elem(c1,c2).real()<<","<<fh_solutions[i].elem(0).elem(s1,s2).elem(c1,c2).imag()<<std::endl;
            ...
            QDPIO::cout << "Starting second set of solves: " << std::endl;
            ...
            for(int i=0; i<1;i++){
                ...
                //pnorm=norm2(fh_prop_solution);
                QDPIO::cout << "Norm["<<i<<"]=" << pnorm<<std::endl;
                //fh_solutions[i]=zero; 
                if(true)
